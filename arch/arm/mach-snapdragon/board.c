 ...
 * the devicetree path, with a few quirks to handle non-standard dtb filenames. This is not
 * meant to be a comprehensive solution to automatically picking the DTB, but aims to be correct
 * for the majority case. For most devices it should be possible to make this algorithm work by
...
static void configure_env(void) {
	...
	strncpy(buf, last_compat, sizeof(buf) - 1);
	tmp = buf;
	...
	strncpy(buf, first_compat, sizeof(buf) - 1);
	tmp = buf;
