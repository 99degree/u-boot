...
/* For simplicitly we only support the case with 1 region */
#define SMEM_MAX_REGIONS 1
...
struct qcom_smem {
	...
	unsigned num_regions; /* Always 1 for now */
...
int qcom_smem_init(void)
{
	...
	mmu_map_region(smem->regions[0].aux_base, reg_size);
