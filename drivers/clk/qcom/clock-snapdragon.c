...
static void dump_rcgs(void) {
	...
	for (i = 0; i < ARRAY_SIZE(rcgs); i++) {
		cmd = readl(rcgs[i]);
		cfg = readl(rcgs[i] + RCG_CFG_REG);
		m = readl(rcgs[i] + RCG_M_REG);
		not_n_minus_m = readl(rcgs[i] + RCG_N_REG);
		...
		printf("%#010x %#010x %#010x %#010x %#010x\n", cmd, cfg, m, not_n_minus_m, readl(rcgs[i] + RCG_D_REG));
