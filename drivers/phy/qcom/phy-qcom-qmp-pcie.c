...
static const struct qmp_pcie_init_tbl sm8250_qmp_pcie_serdes_tbl[] = {
...
static const struct qmp_pcie_init_tbl sm8250_qmp_pcie_tx_tbl[] = {
...
static const struct qmp_pcie_init_tbl sm8250_qmp_pcie_rx_tbl[] = {
...
static const struct qmp_pcie_init_tbl sm8250_qmp_pcie_pcs_tbl[] = {
	QMP_PHY_INIT_CFG(QPHY_V4_PCS_P2U3_WAKEUP_DLY_TIME_AUXCLK_L, 0x01),
	QMP_PHY_INIT_CFG(QPHY_V4_PCS_RX_SIGDET_LVL, 0x77),
	QMP_PHY_INIT_CFG(QPHY_V4_PCS_RATE_SLEW_CNTRL1, 0x0b),
};

static const struct qmp_pcie_init_tbl sm8250_qmp_pcie_pcs_misc_tbl[] = {
...
static int qmp_pcie_vreg_init(struct udevice *dev, struct qmp_pcie_priv *qmp)
{
	...
	for (vreg = 0; vreg < NUM_SUPPLIES && vreg < qmp->vreg_count; ++vreg) {
		...
		ret = regulator_set_enable(qmp->vregs[vreg], true);
		if (ret)
			dev_err(dev, "failed to enable regulator %d (ret=%d)\n", vreg, ret);
	}
...
static int qmp_pcie_reset_init(struct udevice *dev, struct qmp_pcie_priv *qmp)
{
	...
	if (cfg->has_nocsr_reset) {
		...
		if (ret) {
			pr_err("%s: failed to get nocsr reset\n", __func__);
		}
	}
...
/*
 * The PCIe phy exports some fixed clocks, register a stub so the pcie driver
 * can clk_get_bulk()
 */
static int qmp_pcie_bind(struct udevice *dev)
{
	struct driver *drv;
	ofnode node = dev_ofnode(dev);

	drv = lists_driver_lookup_name("clk_stub");
	if (!drv)
		return -ENOENT;
...
static const struct udevice_id qmp_pcie_ids[] = {
	...
	{ .compatible = "qcom,sm8650-qmp-gen3x2-pcie-phy", .data = (ulong)&sm8550_qmp_gen3x2_pciephy_cfg, },
	{ .compatible = "qcom,sm8650-qmp-gen4x2-pcie-phy", .data = (ulong)&sm8650_qmp_gen4x2_pciephy_cfg },
	{ }
};
